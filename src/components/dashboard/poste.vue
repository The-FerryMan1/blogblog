        ...
        <section class="flex border-t-2 border-slate-800 mt-2 p-1 justify-between items-center">
            <div class="flex space-x-7 items-center">
                <button @click="likeHandler(data?.post.id)" type="button"
                    :class="data.post?.likes?.includes(auth.user.email) ? 'text-sm text-white py-2 px-5 bg-blue-600 relative border-2 mt-2 rounded-lg  border-slate-800' : 'text-sm text-white py-2 px-5 bg-slate-600 relative  mt-2 rounded-lg border-2 border-slate-800'">
                    <div
